...
def UMLS_Pretrain_dataset_builder(jaon_path):
    pass
...
def m1_UMLS_TaskB_TextClf_train_dataset_builder(json_path):
    ...
    with open(json_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
    print("UMLS training dataset size is: " + str(len(data)))
    ...
    for item in train:
        ...
        context.append(
            f"There are two medical or biological terms.'{parent}':{parent_info} '{child}':{child_info}")
        label.append(s)
...
def m3_UMLS_TaskB_TextClf_train_dataset_builder(json_path):
    # open train-dataset file
    with open(json_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
...
def m4_UMLS_TaskB_TextClf_train_dataset_builder(json_path):
    # open train-dataset file
    with open(json_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
    print("GeoNames training dataset size is: " + str(len(data)))
...
def UMLS_TaskB_TextClf_test_dataset_builder(json_path):
    filename = json_path
...
def UMLS_TaskB_TextClf_evl_dataset_builder(json_path):
    train,evaluation = combinations(json_path)
    label = []
    text = []
    context = []
