...
def GO_Pretrain_dataset_builder(jaon_path):
    pass

def GO_TaskA_CausalLM_dataset_builder(json_path):
    pass
    #
...
def m1_GO_TaskB_TextClf_train_dataset_builder(json_path):
    root_path = find_root_path()
    ...
    with open(json_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
    print("GO training dataset size is: " + str(len(data)))
    ...
    for item in train:
        ...
        context.append(
            f"There are two biological terms in Gene Ontology. '{parent}':{parent_info} '{child}':{child_info}")
        label.append(s)
...
def m3_GO_TaskB_TextClf_train_dataset_builder(json_path):
    # open train-dataset file
    with open(json_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
    print("GO training dataset size is: " + str(len(data)))
...
def m4_GO_TaskB_TextClf_train_dataset_builder(json_path):
    # open train-dataset file
    with open(json_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
    print("GO training dataset size is: " + str(len(data)))
...
def GO_TaskB_TextClf_test_dataset_builder(json_path):
    filename = json_path
...
def GO_TaskB_TextClf_evl_dataset_builder(json_path):
    train,evaluation = combinations(json_path)
    label = []
    text = []
    context = []
